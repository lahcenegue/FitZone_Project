...
class LoyaltyMilestone {
  ...
  factory LoyaltyMilestone.fromDbMap(Map<String, dynamic> map) {
    LoyaltyReward? parsedReward;
    if (map['reward_id'] != null) {
      parsedReward = LoyaltyReward(
        id: int.tryParse(map['reward_id']?.toString() ?? '0') ?? 0,
        name: map['reward_name']?.toString() ?? '',
        actionType: map['reward_action_type']?.toString() ?? '',
        actionValue:
            double.tryParse(map['reward_action_value']?.toString() ?? '0.0') ??
            0.0,
        fulfillmentType:
            map['reward_fulfillment_type']?.toString() ?? 'IMMEDIATE',
        actionRoute: map['reward_action_route']?.toString(),
        constraints: {},
      );
    }
    ...
    return LoyaltyMilestone(
      ...
      requiredLifetimePoints:
          int.tryParse(map['required_lifetime_points']?.toString() ?? '0') ?? 0,
      reward: parsedReward,
  ...
  factory NextMilestone.fromJson(Map<String, dynamic> json) {
    ...
    try {
      return NextMilestone(
        ...
        progressPct:
            double.tryParse(json['progress_pct']?.toString() ?? '0.0') ?? 0.0,
      );
    } catch (e, stackTrace) {
...
class ConsumedDetails {
  ...
  factory ConsumedDetails.fromJson(Map<String, dynamic> json) {
    final Logger logger = Logger('ConsumedDetailsModel');
    try {
      ...
      logger.severe('Error parsing ConsumedDetails', e, stackTrace);
      return const ConsumedDetails(
        locationName: '',
        actionDescription: '',
        transactionRef: '',
      );
...
class UserMilestone {
  ...
  final ConsumedDetails? consumedDetails;
  ...
  factory UserMilestone.fromJson(Map<String, dynamic> json) {
    ...
    try {
      return UserMilestone(
        ...
        consumedDetails: json['consumed_details'] != null
            ? ConsumedDetails.fromJson(
                json['consumed_details'] as Map<String, dynamic>,
              )
            : null,
      );
    } catch (e, stackTrace) {
